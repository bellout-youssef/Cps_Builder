...
import { X, AlertTriangle, CheckCircle, MessageSquare, XCircle } from 'lucide-react';
...
const ACTION_CONFIG: Record<WorkflowAction, ActionConfig> = {
  ...
  [WorkflowAction.SEND_TO_ADMIN]: {
    ...
    description: 'Transmettre le projet à l\'administrateur pour validation.',
    variant: 'primary',
    requireComment: false,
    icon: <CheckCircle className="h-5 w-5 text-indigo-500" />,
  },
  ...
  [WorkflowAction.REQUEST_MODIFICATION]: {
    ...
    description: 'Renvoyer le CPS au créateur avec une demande de modification. Un commentaire est obligatoire.',
    variant: 'secondary',
    requireComment: true,
...
export function ConfirmModal({ action, onConfirm, onClose }: ConfirmModalProps) {
