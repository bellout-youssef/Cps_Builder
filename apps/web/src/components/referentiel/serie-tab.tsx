...
import type { SerieItem } from '@/lib/api/referential';
import { getSeries, createSerie, updateSerie, deleteSerie } from '@/lib/api/referential';
      ...
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="sr-desc" className="text-sm font-medium text-slate-700">
              Description
            </label>
            <textarea
              id="sr-desc"
...
export function SerieTab() {
  ...
  const [editItem, setEditItem] = useState<SerieItem | null>(null);
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<SerieItem | null>(null);
  ...
  async function handleSave(form: FormState) {
    ...
    try {
      ...
      if (editItem) {
        await updateSerie(editItem.id, payload);
      } else {
  ...
  const initial: FormState = editItem
    ? { code: editItem.code ?? '', name: editItem.name, description: editItem.description ?? '' }
    : EMPTY;
  ...
  return (
    <div>
      ...
      {!loading && !error && series.length === 0 && (
        <p className="py-12 text-center text-sm italic text-slate-400">Aucune série</p>
      )}
            ...
            <tbody className="divide-y divide-slate-50">
              {series.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-xs text-indigo-600">{s.code ?? '—'}</td>
