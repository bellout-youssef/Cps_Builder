...
import type { UnitItem } from '@/lib/api/referential';
import { getUnits, createUnit, updateUnit, deleteUnit } from '@/lib/api/referential';
...
const EMPTY: FormState = { label: '', description: '' };
...
export function UnitTab() {
  ...
  async function handleSave(form: FormState) {
    ...
    try {
      ...
      if (editItem) {
        await updateUnit(editItem.id, payload);
      } else {
  ...
  const initial: FormState = editItem
    ? { label: editItem.label, description: editItem.description ?? '' }
    : EMPTY;
  ...
  return (
    <div>
      ...
      {!loading && !error && units.length === 0 && (
        <p className="py-12 text-center text-sm italic text-slate-400">Aucune unité</p>
      )}
            ...
            <tbody className="divide-y divide-slate-50">
              {units.map((u) => (
                <tr key={u.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-500">{u.description ?? '—'}</td>
                  {canManage && (
      ...
      <ConfirmDialog
        ...
        message={`Confirmer la suppression de « ${deleteTarget?.label} » ?`}
